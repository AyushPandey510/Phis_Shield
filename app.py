...
def handle_join_dashboard(data):
    ...
    logger.info(f"Client {request.sid} joined dashboard for user {user_id}")
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Check for extension requests (more permissive detection)
        is_extension_request = (
            origin.startswith('chrome-extension://') or
            'Chrome' in user_agent or  # Allow any Chrome requests
            referer.startswith('chrome-extension://') or
            'Extension' in user_agent
        )
...
def check_url_endpoint():
    ...
    try:
        ...
        # Emit real-time update for high-risk detections
        if risk_score >= 70 and user_id:  # High risk threshold
            detection_data = {
                'user_id': user_id,
                'url': validated_url,
                ...
                'details': details[:3] if details else []  # First 3 details for brevity
            }

            # Emit to user's specific room
            socketio.emit('high_risk_detected', detection_data, room=f"user_{user_id}")
            logger.info(f"Emitted high-risk detection for user {user_id}: {validated_url}")
...
if __name__ == '__main__':
    socketio.run(app, debug=settings.debug, host=settings.host, port=settings.port, allow_unsafe_werkzeug=True)
